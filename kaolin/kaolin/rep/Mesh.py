...
class Mesh():
    ...
    def compute_adjacency_info(vertices: torch.Tensor, faces: torch.Tensor):
        ...
        nb_vertices = vertices.shape[0]  # 点的数目
        nb_faces = faces.shape[0]
        edges = torch.cat([faces[:, i:i + 2] for i in range(facesize - 1)] +
                          [faces[:, [-1, 0]]], dim=0)  # 在dim=0上拼接列(0, 1),(1,2),(-1,0)。边是由两个点组成的
        ...
        # EDGE2EDGES
        _edges_ids = edges_ids.reshape(facesize, nb_faces)  # stack(tensors, dim=0, *, out=None)沿着dim连接成一个sequence
